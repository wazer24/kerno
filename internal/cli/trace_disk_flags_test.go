// Copyright 2026 Optiqor contributors
// SPDX-License-Identifier: Apache-2.0

package cli

import "testing"

// TestNewTraceDiskCmd_Flags verifies the trace disk command
// exposes all documented flags.
func TestNewTraceDiskCmd_Flags(t *testing.T) {
	cmd := newTraceDiskCmd()

	wantFlags := []string{
		"op",
		"threshold",
		"process",
		"duration",
		"output",
	}

	for _, name := range wantFlags {
		if cmd.Flags().Lookup(name) == nil {
			t.Errorf("trace disk cmd missing --%s flag", name)
		}
	}

	// -o shorthand for --output.
	if cmd.Flags().ShorthandLookup("o") == nil {
		t.Error("trace disk cmd missing -o shorthand for --output")
	}
}

func TestNewTraceDiskCmd_Defaults(t *testing.T) {
	cmd := newTraceDiskCmd()

	cases := []struct {
		flag string
		want string
	}{
		{"op", ""},
		{"threshold", "0s"},
		{"process", ""},
		{"duration", "0s"},
		{"output", ""},
	}

	for _, c := range cases {
		f := cmd.Flags().Lookup(c.flag)
		if f == nil {
			t.Errorf("flag %q not found", c.flag)
			continue
		}

		if f.DefValue != c.want {
			t.Errorf("--%s default = %q, want %q",
				c.flag, f.DefValue, c.want)
		}
	}
}
